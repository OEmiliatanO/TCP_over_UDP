...
namespace tcp_connection
{
    ...
    struct connection
    {
        ...
        std::chrono::milliseconds delay_duration = 600ms;
...
RETRIEVE_PACKET:
                    ...
                    if (not detect_gap and this->ack == seq_num)
                    {
                        ...
                        if (packet_cnt == 1)
                        {
                            std::cerr << std::format("thread #{}: wait 600ms for next segment", thread_id) << std::endl;
